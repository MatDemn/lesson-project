...
const ExerciseBar = ({
  ...
  return (
    <div
      className={exerciseBarStyles.mainContent}
      onClick={() => redirectToExercise(id)}
    >
      <div className={exerciseBarStyles.topBar}>
        <div className={exerciseBarStyles.title + " px-2"}>{title}</div>
        <div className={exerciseBarStyles.difficultyBar}>
          {[1, 2, 3, 4, 5].map((i) => (
            <FontAwesomeIcon
              icon={i <= difficulty ? faStar : faStarEmpty}
              size="1x"
      ...
      <div className={exerciseBarStyles.tags}>
        {tags.slice(0, 10).map((elem) => (
          <TagPill content={elem} />
        ))}
      </div>
    </div>
  );
